    ...
    <ul>
      <li v-for="clanak in pagedClanci" :key="clanak.id">
        <router-link :to="'/article/' + clanak.id">
          ...
          <p>Broj posjeta: {{clanak.brojPoseta}}</p>
        </router-link>
      </li>
    </ul>
...
export default {
  ...
  methods: {
    async fetchClanci() {
      try {
        const response = await axios.get('http://localhost:8080/webprojekat/api/articles/last30days');
        const clanci = response.data.sort((a, b) => b.brojPoseta - a.brojPoseta);
